...
import { fetchCategories } from '@/src/services/category/categoryService';
import { updateTodo } from '@/src/services/todo/todoService';
import { fetchLists } from '@/src/services/todo/listService';
import { PRIORITIES, PRIORITY_LABELS } from '@/src/utils/todo';
import type { Priority, Todo, UpdateTodoPayload } from '@/src/types/Todo';
...
export function EditTaskModal({ open, task, onClose }: EditTaskModalProps) {
  ...
  const [taskDate, setTaskDate] = useState(() => task?.dueDate ? task.dueDate.split('T')[0] : '');
  const [taskPriority, setTaskPriority] = useState<Priority>(() => task?.priority || 'MEDIUM');
  const [taskListId, setTaskListId] = useState(() => task?.listUuid || (task as Todo & { listId?: string })?.listId || '');
  const [taskCategoryId, setTaskCategoryId] = useState<string | null>(() => (task as Todo & { categoryId?: string })?.categoryId || null);
  ...
  return (
    <>
      <RetroModal open={open} title="Editar Tarea" onClose={onClose}>
        <div className="flex flex-col gap-4">
          <PixelInput label="Título" value={taskTitle} onChange={setTaskTitle} />
          <PixelInput label="Descripción" value={taskDesc} onChange={setTaskDesc} />
          ...
          <RetroButton
            variant="primary"
            disabled={!isValid || updateTaskMut.isPending}
            onClick={() => {
              ...
              updateTaskMut.mutate({
                ...
                dueDate: taskDate ? new Date(taskDate).toISOString() : undefined,
          ...
          >
            {updateTaskMut.isPending ? 'GUARDANDO…' : 'GUARDAR CAMBIOS'}
          </RetroButton>
        </div>
      </RetroModal>
